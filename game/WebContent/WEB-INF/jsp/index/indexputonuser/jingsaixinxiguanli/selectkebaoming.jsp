...
<html>
    ...
    <body>
        <div class="x-body">
            ...
            <table class="layui-table"   id="number">
                <thead>
                    <tr>
                        <th>
                            比赛名称
                ...
                <tbody>   
	                <c:forEach items="${fabuinfo}" var="fabuinfo">
						<tr>
	                        ...
	                        <td class="td-manage">
	                        	<button class="layui-btn" onclick="baoming(${fabuinfo.id})" type="submit">报名</button>
	                        </td> 
	                    </tr>
                    </c:forEach>
                </tbody>
        ...
        <script>
           ...
           function baoming(fabuinfoid){
        		var params="fabuinfoid="+fabuinfoid;
        		$.ajax({
        			   type: "POST",
        			   url: "${root}ptyh/jsxx/insertbaoming.action",
        			   data: params,
        			   success: function(msg){
        				   if(msg.code==1){
        					   layer.msg(msg.message,{icon:1,time:2000});
        					   window.$("#number").load(window.location.href+" #number");
        					   //alert(msg.message);
        					   }
        				   else if(msg.code==-1){
        					   alert(msg.message);
